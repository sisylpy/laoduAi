        ...
        if (res.result.data.nxDoDepDisGoodsId !== -1) {
          ...
          for (let i = 0; i < depGoodsArrAi.length; i++) {
            if (depGoodsArrAi[i].nxDepartmentDisGoodsId === depDisGoodsId) {
              const updatePath = `depGoodsArrAi[${i}].nxDepartmentOrdersEntity`;
              this.setData({
    ...
    updateOrder(dg).then(res => {
      ...
      if (res.result.code == 0) {
        ...
        if (this.data.tab1Index == 0) {
          // var orderData = "depGoodsArrAi[" + this.data.editOrderIndex + "].nxDepartmentOrdersEntity";
          // this.setData({
          ...
          var orderData = "goodsList[" + this.data.editOrderIndex + "].nxDepartmentOrdersEntity";
          this.setData({
            [orderData]: res.result.data
          })

          // 新增：如果订单有 depDisGoodsId，则同步更新 depGoodsArrAi
          if (res.result.data.nxDoDepDisGoodsId !== -1) {
            const depGoodsArrAi = this.data.depGoodsArrAi || [];
            const depDisGoodsId = res.result.data.nxDoDepDisGoodsId;
            for (let i = 0; i < depGoodsArrAi.length; i++) {
    ...
    deleteOrder(this.data.applyItem.nxDepartmentOrdersId).then(res => {
      if (res.result.code == 0) {
        ...
        if (that.data.tab1Index == 0) {
          ...
          // 从 depGoods 中获取部门商品ID，而不是从 applyItem 中获取
          const depDisGoodsId = this.data.depGoods ? this.data.depGoods.nxDepartmentDisGoodsId : null;
          const orderId = deletedOrder.nxDepartmentOrdersId;
          const depGoodsArrAi = this.data.depGoodsArrAi || [];
          
          console.log('删除的订单信息:', {
            depDisGoodsId: depDisGoodsId,
            orderId: orderId,
            depGoodsArrAiLength: depGoodsArrAi.length,
            depGoods: this.data.depGoods
          });

          for (let i = 0; i < depGoodsArrAi.length; i++) {
            console.log(`检查 depGoodsArrAi[${i}]:`, {
              nxDepartmentDisGoodsId: depGoodsArrAi[i].nxDepartmentDisGoodsId,
              depGoodsDepOrderListLength: depGoodsArrAi[i].depGoodsDepOrderList ? depGoodsArrAi[i].depGoodsDepOrderList.length : 0
            });
          ...
          // 信用客户页面不需要更新 goodsList，因为 goodsList 为空
        ...
        if (that.data.tab1Index == 1) {
          ...
          // 新增：同步清空 depGoodsArrAi
          const deletedOrder = this.data.applyItem;
          console.log('删除订单时 applyItem:', deletedOrder);

          if (deletedOrder && deletedOrder.nxDoDepDisGoodsId !== undefined && deletedOrder.nxDoDepDisGoodsId !== -1) {
            const depGoodsArrAi = this.data.depGoodsArrAi || [];
            const depDisGoodsId = deletedOrder.nxDoDepDisGoodsId;
            console.log('depGoodsArrAi 长度:', depGoodsArrAi.length, 'depDisGoodsId:', depDisGoodsId);

            let found = false;
            for (let i = 0; i < depGoodsArrAi.length; i++) {
              console.log(`检查 depGoodsArrAi[${i}]:`, depGoodsArrAi[i].nxDepartmentDisGoodsId);
              if (depGoodsArrAi[i].nxDepartmentDisGoodsId === depDisGoodsId) {
                const updatePath = `depGoodsArrAi[${i}].nxDepartmentOrdersEntity`;
                console.log('找到匹配，清空路径:', updatePath);
                this.setData({
                  [updatePath]: null
                });
                found = true;
            ...
            if (!found) {
              console.log('depGoodsArrAi 中未找到匹配的商品');
            }
          } else {
            console.log('deletedOrder.nxDoDepDisGoodsId 无效，不做 depGoodsArrAi 清空');
          }
        }
        that._cancle()
